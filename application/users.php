...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['button-upd'])) {
    ...
    if ($pass_first !== $pass_second) {
        ...
        // Проверяем, был ли введен новый пароль, если да, то хешируем и помещаем в
        ...
        // Проверяем, существует ли пользователь с таким email, исключая текущего
        ...
        if ($check_email_result && $check_email_result->num_rows > 0) {
            ...
            $update_query = "UPDATE users SET us_name = '$us_name', email = '$email',
= $age $password_update, info = '$info_escaped' WHERE id = $id";
